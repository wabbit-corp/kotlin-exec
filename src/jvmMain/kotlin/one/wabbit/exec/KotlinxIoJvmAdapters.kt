 ...
 *
 * The returned input opens the stream when stdin pumping starts, copies it to child stdin, and
 * closes it after copying. Use this adapter when call sites want to keep the common [ExecSpec] API
 * but their data source is JVM stream-based.
 *
...
fun execInputWriteTo(write: (OutputStream) -> Unit): ExecSpec.Input =
    ExecSpec.Input.WriteTo { sink -> write(sink.asOutputStream()) }
